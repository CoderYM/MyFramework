 ...
 */
@MyBatisRepository 
public interface IFrameworkSysActionDao  {

	/**根据自定义的对象查询,返回对象集合
	 * 该方法尚未在Mybatis的Mapper文件中进行配置,请自行实现
	 */
	public List<FrameworkSysAction> findListByObj(FrameworkSysAction frameworkSysAction);
	/**
	 * 根据自定义的sql查询对象返回对象集合
	 * 该方法尚未在Mybatis的Mapper文件中进行配置,请自行实现
	 */
	public List<FrameworkSysAction> findListBySqlId(String sqlId,FrameworkSysAction frameworkSysAction);
	public  FrameworkSysAction findByPKId(@Param("pk_Id") int pk_Id);
