 ...
 */
@MyBatisRepository 
public interface IFrameworkUserRoleDao  {

	/**根据自定义的对象查询,返回对象集合
	 * 该方法尚未在Mybatis的Mapper文件中进行配置,请自行实现
	 */
	public List<FrameworkUserRole> findListByObj(FrameworkUserRole frameworkUserRole);
	/**
	 * 根据自定义的sql查询对象返回对象集合
	 * 该方法尚未在Mybatis的Mapper文件中进行配置,请自行实现
	 */
	public List<FrameworkUserRole> findListBySqlId(String sqlId,FrameworkUserRole frameworkUserRole);
	public  FrameworkUserRole findByPKId(@Param("pk_Id") int pk_Id);
	
	public void updateFrameworkUserRoleBySql(FrameworkUserRole frameworkUserRole);
